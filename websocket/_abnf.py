...
class ABNF:
    ...
    def validate(self, skip_utf8_validation=False) -> None:
        ...
        if self.opcode not in ABNF.OPCODES:
            raise WebSocketProtocolException(
                "Invalid opcode {opcode}".format(opcode=self.opcode)
            )
        ...
        if self.opcode == ABNF.OPCODE_CLOSE:
            ...
            if not self._is_valid_close_status(code):
                raise WebSocketProtocolException(
                    "Invalid close opcode {opcode}".format(opcode=code)
                )
    ...
    def __str__(self) -> str:
        return f"fin={str(self.fin)} opcode={str(self.opcode)} data={str(self.data)}"
